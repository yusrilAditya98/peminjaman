...
class M_Peminjaman extends CI_Model
{
	...
	public function getData($rowno, $rowperpage, $search = "", $pengguna = "", $penyelenggara = "", $jenis = "", $status = "", $tgl_mulai = "", $tgl_selesai = "")
	{
		$operator = $this->session->userdata('username');
		$this->db->select('peminjaman.*,mahasiswa.*,ruangan.*,barang.*,sarana_peminjaman.alasan_penolakan');
		$this->db->from('peminjaman');
		$this->db->join('mahasiswa', 'peminjaman.id_peminjam = mahasiswa.id_mahasiswa', 'left');
		$this->db->join('sarana_peminjaman', 'peminjaman.id_peminjaman = sarana_peminjaman.id_peminjaman', 'left');
		$this->db->join('ruangan', 'ruangan.id_ruangan = sarana_peminjaman.id_sarana', 'left');
		...
		if ($pengguna != '') {
			$this->db->where('peminjaman.nama_peminjam', $search);
		}
		if ($penyelenggara != '') {
	...
	public function getrecordCount($search = '',	$pengguna = '', $penyelenggara = '', $jenis = '', $status = '', $tgl_mulai = '', $tgl_selesai = '')
	{
		$operator = $this->session->userdata('username');
	...
	{
		$operator = $this->session->userdata('username');
		$this->db->select('peminjaman.*,ruangan.*,barang.*,waktu.*,sarana_peminjaman.alasan_penolakan');
		$this->db->from('peminjaman');
		$this->db->join('sarana_peminjaman', 'peminjaman.id_peminjaman = sarana_peminjaman.id_peminjaman', 'left');
		$this->db->join('ruangan', 'ruangan.id_ruangan = sarana_peminjaman.id_sarana', 'left');
		$this->db->join('barang', 'barang.id_barang = sarana_peminjaman.id_sarana', 'left');
	...
	function getDataPeminjamanExcel()
	{
		$this->db->select('*');
		$this->db->from('peminjaman');
		$this->db->join('sarana_peminjaman', 'peminjaman.id_peminjaman = sarana_peminjaman.id_peminjaman');
